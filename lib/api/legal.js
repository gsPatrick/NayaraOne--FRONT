...
export async function createContractVersion(contractId, { content, documentFileId, effectiveFrom, templateId, requireDocument }) {
  return apiFetch(`/legal/contracts/${contractId}/versions`, {
    ...
    body: {
      ...
      ...(typeof requireDocument === "boolean" ? { requireDocument } : {}),
    },
  });
}
...
// GET /legal/contracts/:id/versions/:versionId/generate-pdf devolve os BYTES do PDF direto
// (Content-Type: application/pdf, gerado em memória a partir de ContractVersion.content, sem
// side effect) — apiFetch() assume JSON, então aqui fazemos o fetch cru e devolvemos um Blob,
// no mesmo padrão de getInspectionReportBlob (abaixo).
export async function getContractVersionPdfBlob(contractId, versionId) {
  const session = getSession();
  const headers = {};
  if (session?.accessToken) headers.Authorization = `Bearer ${session.accessToken}`;
  const response = await fetch(`${getBaseUrl()}/legal/contracts/${contractId}/versions/${versionId}/generate-pdf`, { headers });
  if (!response.ok) {
    let message = "Erro ao gerar o PDF da versão do contrato.";
...
export async function listContractTemplates(params = {}) {
  ...
  return apiFetch(`/legal/contract-templates${qs ? `?${qs}` : ""}`);
}

export async function getContractTemplate(id) {
  return apiFetch(`/legal/contract-templates/${id}`);
}

// Preview: só renderiza o texto (não persiste nada). `variables` é um objeto opcional
// { chave: valor } usado para substituir placeholders {{chave}} do texto das cláusulas.
export async function renderContractTemplate(id, variables) {
  return apiFetch(`/legal/contract-templates/${id}/render`, {
    method: "POST",
