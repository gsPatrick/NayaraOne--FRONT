...
// URL da API: por padrão fixa no código (evita esquecer de configurar a env var em cada
// domínio de front que subir), mas respeita NEXT_PUBLIC_API_URL quando definida — necessário
// pra rodar/testar contra uma API local (ex.: NEXT_PUBLIC_API_URL=http://localhost:3011/api/v1
// em .env.local), sem esse fallback é impossível testar mudanças de backend antes do deploy.
function getBaseUrl() {
  return process.env.NEXT_PUBLIC_API_URL || "https://homologacao-nayaraone--api.hpztyd.easypanel.host/api/v1";
}
