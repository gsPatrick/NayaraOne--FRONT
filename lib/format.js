...
// Mesma política de fuso do dateOnlyInputToIso, mas para <input type="datetime-local">, que
// devolve/espera "YYYY-MM-DDTHH:mm" SEM fuso. `new Date(str)` interpretaria essa string no
// fuso IMPLÍCITO de quem executa o código — exatamente a dependência de ambiente que o
// AUD-009 eliminou nos outros campos de data. Aqui a string é sempre lida e escrita como
// horário de São Paulo (UTC-3, fixo).
export function dateTimeLocalInputToIso(value) {
  ...
  if (!year || !month || !day) return undefined;
  return new Date(
    Date.UTC(year, month - 1, day, (hour || 0) + SAO_PAULO_UTC_OFFSET_HOURS, minute || 0, 0, 0)
  ).toISOString();
}

// Caminho inverso: instante ISO -> "YYYY-MM-DDTHH:mm" em horário de São Paulo, pra
// pré-preencher um <input type="datetime-local"> numa tela de edição.
export function isoToDateTimeLocalInput(iso) {
  if (!iso) return "";
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return "";
  const spMillis = d.getTime() - SAO_PAULO_UTC_OFFSET_HOURS * 60 * 60 * 1000;
  return new Date(spMillis).toISOString().slice(0, 16);
}
...
// FIX (reportado pela cliente 18/09/2026): contratos sem "contractNumber" preenchido
// apareciam com label em branco em TODOS os seletores do sistema (vistoria, garantia,
// processo jurídico, entrega de chaves) — o usuário não tinha como saber qual contrato
// estava escolhendo. contractNumber é opcional no cadastro (muitos contratos de teste/homolog
// nunca tiveram um número atribuído), então o label nunca pode depender só dele.
export function formatContractLabel(contract) {
  if (!contract) return "";
  if (contract.contractNumber && contract.contractNumber.trim()) return contract.contractNumber;
  const typeLabel = { SALE: "Venda", LEASE: "Locação", SERVICE: "Prestação de serviço" }[contract.contractType] || contract.contractType || "Contrato";
  const shortId = contract.id ? String(contract.id).slice(0, 8) : "?";
  return `${typeLabel} — ${shortId} (sem número)`;
}
...
// FIX (reportado pela cliente): em <input type="date"> / type="datetime-local">, digitar um
// ano fora do intervalo plausível (ex: um dígito a mais, "92026") faz o navegador reportar
// e.target.value = "" silenciosamente, sem disparar nenhum aviso — o form aceitava esse vazio
// e o botão de submit ficava com estado inconsistente (ou submetia a data como nula) sem
// explicação nenhuma pro usuário. Use isDateInputInvalid(e.target.validity) no onChange/onBlur
// do campo e mostre DATE_INPUT_ERROR_MESSAGE como erro visível abaixo dele.
export function isDateInputInvalid(validity) {
  return !!validity && (validity.badInput || validity.rangeOverflow || validity.rangeUnderflow);
}

export const DATE_INPUT_ERROR_MESSAGE =
  "Data inválida — verifique o ano digitado (use um ano entre 1900 e 2100).";
