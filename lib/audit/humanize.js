...
// Nome amigável por chave de configuração (core.tenant_settings.key) — sem isso, a tela mostra
// a chave técnica em ponto ("legal.clicksign_api_token") crua.
const SETTINGS_KEY_LABELS = {
  "billing.late_fee_percentage": "Multa por atraso",
  "billing.interest_percentage": "Juros de mora",
  "billing.grace_period_days": "Dias de carência",
  "billing.default_index_code": "Índice de reajuste padrão",
  "billing.igpm_mode": "Modo do índice IGPM",
  "billing.fgv_api_token": "Token da API da FGV",
  "mfa.step_up_ttl_minutes": "Duração da janela de MFA",
  "mfa.required_for_sensitive_roles": "MFA obrigatório para perfis sensíveis",
  "legal.signature_provider": "Provedor de assinatura eletrônica",
  "legal.clicksign_api_token": "Token da API Clicksign",
  "legal.clicksign_webhook_secret": "Webhook secret Clicksign",
  "legal.zapsign_api_token": "Token da API ZapSign",
  "legal.zapsign_webhook_secret": "Webhook secret ZapSign",
};

// Chaves marcadas `encrypted: true` no SETTINGS_SCHEMA do backend (settings.service.js) — o
// valor gravado em before/afterJson é o CIPHERTEXT (AES-256-GCM), não o segredo em texto
// claro (nunca há vazamento real aqui), mas mostrar esse blob cru na tela ("RclL2PVL...")
// é confuso e parece um bug. Mostramos só que o valor foi configurado/alterado, nunca o blob.
...
// summarizeSettingChange — trata "core"."tenant_settings" à parte do caminho genérico: o
// registro tem só {key, value}, e "value" pode ser um segredo criptografado que nunca deve
// aparecer cru na tela (ver SETTINGS_SECRET_KEYS acima).
function summarizeSettingChange(beforeJson, afterJson) {
  const before = beforeJson || {};
  const after = afterJson || {};
  const key = after.key || before.key;
  const isSecret = SETTINGS_SECRET_KEYS.has(key);

  const beforeValue = isSecret
    ? (before.value ? "Configurado" : "—")
    : formatValue("value", before.value);
  const afterValue = isSecret
    ? (after.value ? "Configurado" : "—")
    : formatValue("value", after.value);

  if (!beforeJson) {
    return [{ label: "Configuração", after: settingsKeyLabel(key), isNew: true }, { label: "Valor", after: afterValue, isNew: true }];
  }
  if (beforeValue === afterValue) {
    return [{ label: "Configuração", after: settingsKeyLabel(key), isNew: true }];
  }
  return [
    { label: "Configuração", after: settingsKeyLabel(key), isNew: true },
    { label: "Valor", before: beforeValue, after: afterValue, isNew: false },
  ];
}

// Resume before_json/after_json numa lista de { label, before, after, isNew } já em pt-BR,
// pronta pra renderizar como texto simples — nunca como bloco de código. `entityType` permite
// tratamento dedicado quando o registro genérico não faz sentido (ver TenantSetting acima).
export function summarizeChanges(beforeJson, afterJson, entityType) {
  if (entityType === "TenantSetting") return summarizeSettingChange(beforeJson, afterJson);
