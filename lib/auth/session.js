...
const KEYS = {
  ...
  mfaSetupRequired: "nayaraone.auth.mfaSetupRequired",
};
...
export function saveSession({
  ...
  if (mfaSetupRequired) {
    localStorage.setItem(KEYS.mfaSetupRequired, "1");
  } else {
    localStorage.removeItem(KEYS.mfaSetupRequired);
  }
}

// Indica que o login retornou mfaSetupRequired: true (empresa exige MFA para permissões
// sensíveis e o usuário ainda não configurou) — usado pelo AuthGuard para travar a navegação
// em /painel/perfil até a configuração ser concluída.
export function isMfaSetupRequired() {
  if (!isBrowser()) return false;
  return window.localStorage.getItem(KEYS.mfaSetupRequired) === "1";
}

export function clearMfaSetupRequired() {
  if (!isBrowser()) return;
  window.localStorage.removeItem(KEYS.mfaSetupRequired);
}
